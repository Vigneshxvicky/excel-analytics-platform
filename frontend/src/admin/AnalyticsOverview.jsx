...
const SOCKET_SERVER_URL =
  "https://excel-analytics-platform-backend.onrender.com"; // Backend URL
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${SOCKET_SERVER_URL}/api/dashboard/users`,
        {
      ...
      console.log(
        `[Analytics] Start Date (UTC): ${startDate.toISOString()}, Time Range: ${timeRange}, Group By: ${groupByUnit}`
      );

      // Filter users within the time range
      const filteredUsers = users.filter(
        (user) => user.createdAt && new Date(user.createdAt) >= startDate
      ); // Add check for createdAt existence
      console.log(
        `[Analytics] Total Users: ${users.length}, Filtered Users (in range): ${filteredUsers.length}`
      );
      ...
      const countsByPeriod = filteredUsers.reduce((acc, user) => {
        ...
        if (groupByUnit === "hour") {
          ...
          ).toISOString(); // Use UTC components for key
        } else {
          // 'day'
          key = new Date(
            Date.UTC(
      ...
      while (currentDate <= loopEndDate) {
        // Loop up to the start of the last period
        ...
        const key = new Date(
          Date.UTC(
            ...
            groupByUnit === "hour" ? currentDate.getUTCHours() : 0
          )
        ).toISOString(); // Use UTC components for lookup key
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        {error && (
          <p className="text-center my-4 text-red-500">Error: {error}</p>
        )}
        {chartData ? (
          <Line
            data={chartData}
          ...
          !error && (
            <p className="text-center my-4">No analytics data available.</p>
          ) // Show only if no error
        )}
      </div>
    </div>
  );
